...
def datasail_runner(distance_matrix, output_folder, splits=[0.8, 0.2], cpus=1, max_time=600, epsilon=0.1, delta=0.1, solver="SCIP"):
    """
    ...
    splits, _, _ = datasail_main(output=None, techniques=["C1e"], splits=splits, names=["train", "test"], e_type="P", e_data=[(n, "a" * i) for i, n in enumerate(dm.columns)], e_dist=Path(distance_matrix), max_sec=max_time, threads=cpus, inter=None, max_sol=10, verbosity="I", delta=delta,
                                 epsilon=epsilon, runs=1, solver=solver, cache=False, cache_dir=None, linkage="average", e_weights=None, e_strat=None, e_sim=None, e_args="", e_clusters=50, f_type=None, f_data=None, f_weights=None, f_strat=None, f_sim=None, f_dist=None, f_args="", f_clusters=50, cli=False, logdir=None)
