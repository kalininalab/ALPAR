...
def get_susceptible_cluster_files(wildcards):
    panaroo_checkpoint = checkpoints.panaroo_cluster.get(**wildcards).output[0]
    cluster_file = Path(panaroo_checkpoint) / wildcards.antibiotic / "Susceptible" / f"{wildcards.cluster_gene}.fasta"
    return cluster_file

rule mafft_align_susceptible:
    input: get_susceptible_cluster_files,
    output: OUT_DIR / "protein_align_susceptibles" / "{antibiotic}" / "{cluster_gene}.aln",
    log: TEMP_DIR / "logs" / "mafft_align_susceptible" / "{antibiotic}_{cluster_gene}.log"
    benchmark: TEMP_DIR / "benchmarks" / "protein_alignment_{antibiotic}_Susceptible_{cluster_gene}.tsv"
...
def get_resistant_cluster_files(wildcards):
    panaroo_checkpoint = checkpoints.panaroo_cluster.get(**wildcards).output[0]
    cluster_file = Path(panaroo_checkpoint) / wildcards.antibiotic / "Resistant" / f"{wildcards.cluster_gene}.fasta"
    return cluster_file

rule mafft_add_resistant:
    input:
        alignment = rules.mafft_align_susceptible.output,
        resistant_seq = get_resistant_cluster_files,
    output:
        alignment = OUT_DIR / "protein_alignments_all" / "{antibiotic}" / "{cluster_gene}.aln",
        variation_map = TEMP_DIR / "protein_alignments_map" / "{antibiotic}" / "{cluster_gene}.fasta.map",
    log: TEMP_DIR / "logs" / "mafft_add_resistant" / "{antibiotic}_{cluster_gene}.log"
    benchmark: TEMP_DIR / "benchmarks" / "protein_alignment_{antibiotic}_with_resistant_{cluster_gene}.tsv"
    conda: "envs/mafft.yaml"
    threads: 1
    shell:
        r"""
        ...
        mkdir -p $(dirname {output.variation_map})
        mkdir -p $(dirname {log})

        mafft \
            --auto \
            --thread {threads} \
            --add {input.resistant_seq} \
            --mapout \
            --keeplength \
            {input.alignment} \
            > {output.alignment} 2>> {log}
        
        # Map is created in the same folder as the file in --add
        mv {input.resistant_seq}.map {output.variation_map}
        """
